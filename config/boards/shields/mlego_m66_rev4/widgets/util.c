...
void draw_battery(lv_obj_t *canvas, const struct status_state *state) {
    ...
    // Battery body from src_x = 1 to 29 (left-aligned at column 1)
    canvas_draw_rect(canvas, 1, 2, 29, 12, &rect_white_dsc);
    canvas_draw_rect(canvas, 2, 3, 27, 10, &rect_black_dsc);
    ...
    if (fw > 0) {
        canvas_draw_rect(canvas, 3, 4, fw, 8, &rect_white_dsc);
    }

    // Battery positive terminal at src_x = 30 to 32
    ...
    if (state->charging) {
        lv_draw_image_dsc_t img_dsc;
        lv_draw_image_dsc_init(&img_dsc);
        canvas_draw_img(canvas, 10, -1, &bolt, &img_dsc);
    }
}
