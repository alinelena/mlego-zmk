...
void state_widget_wpm(struct mlego_bongo_cat_widget *widget, int wpm) {
  LOG_DBG("anim state %d", current_anim_state);
  if (wpm < CONFIG_MLEGO_BONGO_CAT_IDLE_LIMIT) {
...
int wpm_status_listener(const zmk_event_t *eh) {
    struct mlego_bongo_cat_widget *widget;
...
ZMK_LISTENER(mlego_bongo_cat_widget, wpm_status_listener)
ZMK_SUBSCRIPTION(mlego_bongo_cat_widget, zmk_wpm_state_changed);
